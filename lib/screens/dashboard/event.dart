...
class _EventState extends State<Event> {
  List<Map> events = [];
  List<Map> filteredEvents = [];
  bool initEvent = false;

  void filteredEventsByType(String criteria) {
    setState(() {
      filteredEvents = events.where((event) {
        if (criteria == 'All Event') {
          return true;
        } else if (criteria == 'Webinar') {
          return event['event'] is Webinar;
        } else if (criteria == 'Bootcamp') {
          return event['event'] is Bootcamp;
        }
        return false;
      }).toList();
    });
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
        stream:
            FirestoreService(uid: widget.user.uid).webinarAndBootcampStream(),
        builder: (BuildContext context, snapshot) {
          if (snapshot.connectionState == ConnectionState.active &&
              !initEvent) {
            initEvent = true;
            Future.delayed(const Duration(milliseconds: 300),
                () => filteredEventsByType('All Event'));
          }
          if (snapshot.hasData) {
            final List<Map> dataMaps = snapshot.data!;
            final dataEvents = dataMaps.map((data) {
              return {
                'id': data['id'],
                'event': data['webinar'] ?? data['bootcamp'],
              };
            }).toList();
            events = List.from(dataEvents);
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(
                          height: filteredEvents.length / 5 > 1 &&
                                  filteredEvents.length / 5 < 2
                              ? height / 1.7
                              : filteredEvents.length / 5 >= 2
                                  ? height / 2.5 * (filteredEvents.length / 5)
                                  : height / 1.6,
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      padding: const EdgeInsets.symmetric(
                                          vertical: 15),
                                      child: Text(
                                        "You don't have any event, \n you can search for events according to your needs",
                              ...
                              : ScrollConfiguration(
                                  ...
                                  child: MasonryGridView.count(
                                    ...
                                    itemBuilder:
                                      ...
                                      // Assuming you have a way to determine if the event is a Bootcamp or Webinar
                                      if (event['event'] is Bootcamp) {
                                        return Bootcamps(
                                            id: event['id'],
                                            bootcamp: event['event']);
                                      } else if (event['event'] is Webinar) {
                                        return Webinars(
                                            id: event['id'],
                                            webinar: event['event']);
                                      } else {
