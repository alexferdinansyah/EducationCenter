...
import 'package:project_tc/components/videoLearning_form_modal.dart';
...
class AdminVideolearning extends StatefulWidget {
  const AdminVideolearning({super.key});

  @override
  State<AdminVideolearning> createState() => _AdminVideolearningState();
}

class _AdminVideolearningState extends State<AdminVideolearning> {
  List<Map> videoLearnings = [];
  List<Map> filteredVideoLearnings = [];
  bool initVideoLearning = false;

  void filterVideoLearnings(String criteria) {
    setState(() {
      filteredVideoLearnings = videoLearnings.where((videoLearning) {
        if (criteria == 'All Video Learnings') {
          return true;
        } else if (criteria == 'Draft') {
          return videoLearning['videoLearning'].isDraft == true;
        }
        return false;
      }).toList();
    });
  ...
  void updateInitVideoLearning(bool newValue) {
    setState(() {
      initVideoLearning = newValue;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      stream: fireStore.allVideoLearning,
      builder: (BuildContext context, snapshot) {
        // print('data');
        // print(snapshot);
        if (snapshot.connectionState == ConnectionState.active &&
            !initVideoLearning) {
          initVideoLearning = true;
          Future.delayed(const Duration(milliseconds: 300),
              () => filterVideoLearnings('All Video Learnings'));
        }
        if (snapshot.hasData) {
          final List<Map> dataMaps = snapshot.data!;
          final dataVideoLearnings = dataMaps.map((data) {
            return {
              'id': data['id'],
          ...
          videoLearnings = List.from(dataVideoLearnings);
                                    ...
                                    children: [
                                      ...
                                      Padding(
                                        padding: const EdgeInsets.symmetric(
                                            vertical: 15),
                                        child: Text(
                                          "There's no video learning in here, click button below to create video learning",
                                        ...
                                        return MyVideoLearning(
                                          videoLearning:
                                              filteredVideoLearnings[index]
                                                  ['videoLearning'],
                                          id: filteredVideoLearnings[index]
                                              ['id'],
                                          isAdmin: true,
                                          onPressed: () async {
                                            final result = await showDialog(
                                              context: context,
                                              builder: (_) {
                                                return VideolearningFormModal(
                                                  videoLearning:
                                                      filteredVideoLearnings[
                                                              index]
                                                          ['videoLearning'],
                                                  videoLearningId:
                                                      filteredVideoLearnings[
                        ...
                        child: FloatingActionButton(
                          onPressed: () async {
                            final result = await showDialog(
                              context: context,
                              builder: (_) {
                                return const VideolearningFormModal();
                              },
                            );
                            if (result != null) {
                              setState(() {
